...
const useChat = (): UseChatReturn => {
  ...
  const loadSession = useCallback(
    async (sessionId: string) => {
      ...
      try {
        ...
        if (!response.ok)
          throw new Error("Failed to fetch messages for selected session.");
        ...
        const newSession: ChatSession = {
          ...
          createdAt: sessionInfo?.createdAt
            ? new Date(sessionInfo.createdAt)
            : new Date(),
  ...
  const sendMessage = async (messageText: string) => {
    ...
    const endpoint = session.userId ? "/api/chat/message" : "/api/chat/guest";
    ...
    try {
      ...
      if (!session.title || session.title === "Guest Session") {
        ...
        const titleSnippet =
          cleanSnippet.length > 0 ? cleanSnippet : "New chat";
        ...
        if (session.userId) {
          ...
          try {
            ...
            // Update session list for authenticated users
            setSessionList((prev) =>
              prev.map((s) =>
                s.id === session.id ? { ...s, title: titleSnippet } : s
              )
            );
          } catch (err) {
        ...
        // Always update local session state (for both guest and authenticated)
        setSession((prev) => (prev ? { ...prev, title: titleSnippet } : prev));
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!currentUserId) {
      const savedGuestChat = sessionStorage.getItem("guestChat");
      if (savedGuestChat) {
        const { session, chatHistory } = JSON.parse(savedGuestChat);
        setSession(session);
  ...
  }, [chatHistory, session, currentUserId]);
